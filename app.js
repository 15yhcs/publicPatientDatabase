...
app.get("/index",checkAuthenticated, function(req,res) {
    ...
    res.render("index.ejs", { name: "names" });
})
...
app.get('/profile', checkAuthenticatedGoogle , (req, res)=>{
    ...
    result.then(data => {
            for (var i = 0; i < data.length; i++){
                ...
                if (data[i].email === req.user.email){
...
app.post("/createProfile",function(req,res){
    ...
    if (eDate){
        ...
        var servingDuration = null;
    }
    var today = new Date();
    console.log(today);
    var bday = moment(birthday, 'YYYY-MM-DD HH:mm:ss');
    var today = moment(today, 'YYYY-MM-DD HH:mm:ss');
    var age = moment.duration(today.diff(bday)).asYears();

    var starDuration = moment.duration(start_date.diff(bday)).asDays();

    const db = DBService.getDbServiceInstance();
    const result = db.checkExistingP(patientNum);
	result.then(data => {
        if(data.length > 0){
        ...
        else if (starDuration<0){
            return res.status(400).send('Invalid starting date, cannot start before born!');
            //console.log("invalid starting date, cannot start before born!");
        }
        else if (servingDuration < 0){
            return res.status(400).send('Invalid starting date and ending date!!');
            //console.log("Invalid starting date and ending date!");
        }
        else{
            ...
            return res.status(400).send('Successfully created the patient profile!');
        }
    });
});
...
app.post("/uploadDoc", function(req,res){
    ...
    if(req.files){
        ...
        console.log(file_name, file_type, jaso_file);
        const db = DBService.getDbServiceInstance();
...
app.post("/createSession",function(req,res){
    ...
    var sDate = moment(startDate, 'YYYY-MM-DD HH:mm:ss');
    var eDate = moment(endDate, 'YYYY-MM-DD HH:mm:ss');
    var duration = moment.duration(eDate.diff(sDate)).asDays();
    const db = DBService.getDbServiceInstance();
    const result = db.checkExistingS(id);
	result.then(data => {
        if(data.length > 0){
            return res.status(400).send('Session ID already exist!');
        }
        else if (duration<0){
            return res.status(400).send('Invalid starting date and ending date!');
        }
        else if(startTime > endTime){
            return res.status(400).send('Invalid starting time and ending time!');
        }
        else{
            ...
            return res.status(400).send('Successfully created the session');
        }
    });        
...
app.post("/createCourse",function(req,res){
    ...
    const db = DBService.getDbServiceInstance();
    const result = db.checkExistingC(id);
	result.then(data => {
        if(data.length > 0){
            //console.log("Course id already exist!");
            return res.status(400).send('The course ID already exists!');
        }
        else{
            db.createCourse(id,name,spot);
            return res.status(400).send('Successfully created the course!');
        }
    });
...
app.post("/addIntoCourse",function(req,res){
    var content = req.body.addContent;    if(content.length>0){
        var result_int = [];
        for (let i = 0; i < content.length; i++) {
            var temp = [];
            for (let j = 0; j < content[i].length; j++) {
            temp.push(content[i][j]);
            }
        ...
        const db = DBService.getDbServiceInstance();
        const checkTaken = db.checkAlreadyTaken(result_int[0][0],result_int[0][1]);
        checkTaken.then(data => {
            if(data.length > 0){
                console.log('Patient already takes this course');
                //res.redirect("/success");
                res.status(400).send('Patient already takes this course');
            }
            else{
                const checkWaiting = db.checkAlreadyWaited(result_int[0][0],result_int[0][1]);
                checkWaiting.then(data => {
                    if(data.length > 0){
                        console.log('Patient already in the wait list');
                        return res.status(400).send('Patient already in the wait list');
                    }
                    else{
            ...
                                        //res.json({success: true})
                                        console.log("Successfully add the patient to the course");
                                        return res.status(400).send('Add patient to course successful');
            ...
                                        //res.json({delete: true})
                                        console.log("The course is full. The patient is in the wait list.");
                                        return res.status(400).send('The course is full. The patient is in the wait list.');
                                    }
                                })
                            ).catch(error => console.log(error));   
                    } 
        ...
        return res.status(400).send('Cannot submit empty input');
    }
})
...
app.post("/addIntoSession",function(req,res){
    ...
        const db = DBService.getDbServiceInstance();
        console.log(result_int);
        for (let i = 0; i < result_int.length; i++){
            console.log(result_int[i]);
            const checkParticipated = db.checkAlreadyParticipated(result_int[i][0],result_int[i][1]);
            checkParticipated.then(data => {
                if(data.length > 0){
                    //return res.status(302).send('Patient " + result_int[i][0] + " already participated this session.');
                    console.log("Patient " + result_int[i][0] + " already participated this session");
                }
                else{
                    db.addIntoSession(result_int[i][0],result_int[i][1]);
                    //return res.status(400).send('Add patient to session successful!');
                }
            })
        } 
})
...
app.post("/addAdmin", async function(req,res){
    ...
    const Fname = req.body.Fname
    const db = DBService.getDbServiceInstance();
    db.addAdmin(id,hashPassword,1,userEmail,Fname,Lname);
    return res.status(400).send('New admin is added');
})
