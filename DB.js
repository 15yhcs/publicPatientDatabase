...
class DBService{
    ...
    async checkExistingP(patientNum){
        try{
            const response = await new Promise((resolve, reject) => {
                const query = 'SELECT * FROM patient WHERE PatientNum = ?';
                connection.query(query,[patientNum], (error,result) => {
                    if(error){
                        reject(new Error(error.message));
                    }
                    resolve(result);
    ...
    async insertNewDoc(file_name, file_type, file){
        try{
            var query = "INSERT INTO document (name, type, fileConent) VALUES (?,?,?)";
    ...
    async checkAlreadyTaken(p,c){
        try{
            const response = await new Promise((resolve, reject) => {
                const query = 'SELECT * FROM Taken WHERE patient = ? AND course = ?';
                connection.query(query,[p,c], (error,result) => {
                    if(error){
                        reject(new Error(error.message));
                    }
                    resolve(result);
    ...
    async checkAlreadyWaited(p,c){
        try{
            const response = await new Promise((resolve, reject) => {
                const query = 'SELECT * FROM Waiting WHERE patient = ? AND course = ?';
                connection.query(query,[p,c], (error,result) => {
                    if(error){
                        reject(new Error(error.message));
                    }
    ...
    async checkAlreadyParticipated(p,s){
        try{
            const response = await new Promise((resolve, reject) => {
                const query = 'SELECT * FROM participate WHERE patient = ? AND session = ?';
                connection.query(query,[p,s], (error,result) => {
                    if(error){
                        reject(new Error(error.message));
                    }
                    resolve(result);
